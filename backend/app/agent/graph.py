...
3. 涉及最新或外部信息（新闻时事、公开数据、政策法规、行业动态等）时，先用
   web_search 检索再写作，产出中注明数据来源与日期，不凭记忆编造数据。
4. 收到以 [修改要求] 或 [用户补充指令] 开头的消息时，按其调整后重新保存（会形成新版本）。
5. 工具保存完成后，用一两句话说明产出了什么即可，不必复述全部内容。"""
