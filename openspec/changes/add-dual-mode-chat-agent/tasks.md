...
- [ ] 2.2 数据隔离校验：会话/任务/产物查询强制 user_id 过滤，集成测试覆盖越权场景（随 G3 端点落地）
...
- [ ] 4.1 任务/闲聊分类器（规则优先起步，预留小模型分类接口），输出结构化判定
- [ ] 4.2 路由集成：普通对话模式下分类 → 任务型返回确认卡片事件（SSE）
- [ ] 4.3 确认/拒绝端点：确认则携带原消息进入工作模式，拒绝则原消息走直答
- [ ] 4.4 前端确认卡片组件与两种点击流转

## 5. Agent 执行核心（agent-task-execution）

- [ ] 5.1 **Spike：LangGraph 执行中消息注入**——验证工具循环间隙检查消息队列并注入的可行方案，结论回写 design.md
- [ ] 5.2 LangGraph 图搭建：execute 节点（工具循环）+ interrupt 预览节点 + finalize 节点，接入 AsyncPostgresSaver
- [ ] 5.3 任务状态机持久化（待命/执行中/预览就绪/已交付/失败）与重启恢复
- [ ] 5.4 执行过程事件流：工具调用/步骤/中间输出经 SSE 推送
- [ ] 5.5 执行中插话链路：消息入队 → 循环间隙注入 → Agent 响应转向
- [ ] 5.6 前端工作模式执行视图：过程事件时间线、可插话输入框
- [ ] 5.7 预览就绪反馈链路：修改指令恢复 thread + 注入反馈（增量修改入口）
...
- [ ] 7.1 沙箱执行服务：按任务创建容器、运行代码、收集 stdout/文件/错误、销毁容器
- [ ] 7.2 加固：无网络、CPU/内存/时长限额、非 root、产出大小限额
- [ ] 7.3 沙箱工具接入 Agent（作为代码执行工具）+ 失败/超时回传处理
- [ ] 7.4 沙箱安全测试：超时终止、网络阻断、残留清理验证
...
- [ ] 9.1 LLM 调用用量埋点（用户/模型/tokens/耗时/会话/任务），直答与 Agent 循环全覆盖
- [ ] 9.2 任务执行统计（时长、工具调用数、沙箱执行数）
- [ ] 9.3 用量汇总查询内部接口（按用户+时间范围）

## 10. 安全加固与端到端验收

- [ ] 10.1 API 限流（按用户/IP）
- [ ] 10.2 端到端场景验收：闲聊直答、路由卡片切换、报告任务（数据→文档）、海报任务、执行中转向、两轮增量修改、满意下载、越权与 XSS 负向用例
- [ ] 10.3 docker-compose 本地一键启动（backend + postgres + sandbox + frontend）
- [ ] 10.4 部署文档：本地运行说明 + 迁服务器配置差异清单
